...
public class Sampling {
    ...
    private boolean weightedSampling;
     ...
     */
    public Sampling(ArrayList<Annotation> goAnnotations, double[][] distance_matrix, boolean _weighted) {
        annotation_goList = goAnnotations;
        distanceMatrix = distance_matrix;
        weightedSampling = _weighted;
        if (weightedSampling) {
     ...
     */
    public void computeMultipleDistributions(int nProtToSampleLowerBound, int nProtToSampleUpperBound, int numOfTimesNetworkIsSampled) {
        ...
        for (int n = nProtToSampleLowerBound; n <= nProtToSampleUpperBound; n++) { // range of proteins to sample
            ...
            if (numberOfAssociatedProteinsExists) {
                ...
                System.out.println("TPD (n = " + n + ")" + "\t" + "Frequency");
                for (double dist : distribution.keySet()) {
                    System.out.println(dist + "\t" + distribution.get(dist));
                }
            } else {
                System.out.println("TPD (n = " + n + ") Not found");
            }
            System.out.println("");
        }
    }
    ...
    private double[] sampleNetwork(int numProteinsToSample, int timesToSampleNetwork) {
        ...
        for (int i = 0; i < timesToSampleNetwork; i++) {
            ArrayList<Integer> randomProteins;
	...
	private HashMap<String, Integer> computeProteinFrequenciesListByGoTermsId() {
        ...
        for (Annotation cluster : annotation_goList) {
            for (int i = 0; i < cluster.getGeneSymbols().size(); i++) {
                String id = cluster.getGeneSymbols().get(i);
